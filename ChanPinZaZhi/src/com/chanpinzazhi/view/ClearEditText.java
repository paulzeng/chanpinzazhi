...
public class ClearEditText extends EditText implements OnFocusChangeListener,
	...
	private void init() {
		...
		if (mClearDrawable == null) {
			mClearDrawable = getResources().getDrawable(R.drawable.btn_search);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (getCompoundDrawables()[2] != null) {
			if (event.getAction() == MotionEvent.ACTION_UP) {
				...
				if (touchable) {
					// this.setText("");
					String key = this.getText().toString();
					initKeySearch(key, handler);
				}
			}
		}
	 ...
	 * @param key
	 */
	void initKeySearch(String key, Handler mHandler) {
		SoapHttpRequest request = new SoapHttpRequest(this.getContext()); 
		request.GetByNavigateId(
				SoapConstants.GETSEARCH,
				this.getContext(),
				mHandler,
				DeviceManager.getMyUUID(this.getContext()),
				4, 
				2,
				"[Id],[App_TypeName],[Name],[App_Introduce],[App_BigImageUrl],[RelatedImages]",
				"[App_IsShow]=1 and [Name] like '%" + key + "%'",
